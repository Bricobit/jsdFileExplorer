...
rem Obtiene la ruta del directorio donde se ejecuta el archivo .bat
set thisFolder=%~dp0
...
echo     ^<meta http-equiv="Content-Security-Policy" content="default-src * 'unsafe-inline' 'unsafe-eval'; img-src * data:; media-src *; script-src * 'unsafe-inline' 'unsafe-eval'; style-src * 'unsafe-inline'; font-src *; object-src *; frame-ancestors *; worker-src * blob:;"^> >> "%indexFile%"
...
echo         #codeContainer {border: 1px solid #ddd; padding: 10px; height: calc(100vh - 40px); overflow-y: auto;} >> "%indexFile%"
echo     ^</style^> >> "%indexFile%"
echo ^</head^> >> "%indexFile%"
echo ^<body^> >> "%indexFile%"
echo ^<nav^> >> "%indexFile%"
...
rem Recorre los archivos .js en source y genera la estructura HTML
for /r %source% %%f in (*.js) do (
    ...
    echo         ^<li^>^<a href="#" onclick="loadCode('!destination!/!relativePath!')"^>📄 !filename!^</a^>^</li^> >> "%indexFile%"
)
...
echo ^</nav^> >> "%indexFile%"
echo ^<main^> >> "%indexFile%"
echo     ^<div id="codeContainer" style="width:100%%; height:100%%;"^>^</div^> >> "%indexFile%"
echo ^</main^> >> "%indexFile%"
...
echo     } >> "%indexFile%"
echo     function loadCode(filePath) { >> "%indexFile%"
echo         fetch(filePath) >> "%indexFile%"
echo         .then(response => response.text()) // Obtiene el texto de la respuesta >> "%indexFile%"
echo         .then(code => { // Manejo del código >> "%indexFile%"
echo             let syntaxHighlighter = new SyntaxColor(); >> "%indexFile%"
echo             document.getElementById('codeContainer').innerHTML = syntaxHighlighter.highlight(code, 'html'); >> "%indexFile%"
echo         }) >> "%indexFile%"
echo         .catch(error => console.error('Error loading code:', error)); // Manejo de errores >> "%indexFile%"
echo     } >> "%indexFile%"
echo ^</script^> >> "%indexFile%"
echo ^</body^> >> "%indexFile%"
echo ^</html^> >> "%indexFile%"
